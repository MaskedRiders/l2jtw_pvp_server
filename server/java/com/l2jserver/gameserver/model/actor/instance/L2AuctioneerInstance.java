...
public final class L2AuctioneerInstance extends L2Npc
{
	...
	public void onBypassFeedback(L2PcInstance player, String command)
	{
		...
		{
			// TODO: html
			/* MessageTable.Messages[534]
			player.sendMessage("Wrong conditions.");
			 */
			player.sendMessage(534);
			return;
		}
		...
		else if (condition == COND_REGULAR)
		{
			...
			if (actualCommand.equalsIgnoreCase("auction"))
			{
				...
				try
				{
					...
					{
						/* MessageTable.Messages[535]
						player.sendMessage("Invalid bid!");
						 */
						player.sendMessage(535);
					}
				}
				catch (Exception e)
				{
					/* MessageTable.Messages[536]
					player.sendMessage("Invalid auction duration!");
					 */
					player.sendMessage(536);
				}
				return;
			}
			else if (actualCommand.equalsIgnoreCase("confirmAuction"))
			{
				...
				{
					/* MessageTable.Messages[537]
					player.sendMessage("Invalid auction");
					 */
					player.sendMessage(537);
				}
				return;
			}
			else if (actualCommand.equalsIgnoreCase("bidding"))
			{
				...
				try
				{
					...
					if (a != null)
					{
						 ...
						 */
						html.replace("%AGIT_AUCTION_REMAIN%", String.valueOf((a.getEndDate() - System.currentTimeMillis()) / 3600000) + MessageTable.Messages[538].getExtra(1) + String.valueOf((((a.getEndDate() - System.currentTimeMillis()) / 60000) % 60)) + MessageTable.Messages[538].getExtra(2));
						html.replace("%AGIT_AUCTION_MINBID%", String.valueOf(a.getStartingBid()));
			...
			else if (actualCommand.equalsIgnoreCase("list"))
			{
				...
				for (int j = 1; j <= npage; j++)
				{
					 ...
					 */
					items.append("\">" + MessageTable.Messages[539].getMessage());
					items.append(j);
					items.append(MessageTable.Messages[539].getExtra(1) + "</a></center></td>");
				}
